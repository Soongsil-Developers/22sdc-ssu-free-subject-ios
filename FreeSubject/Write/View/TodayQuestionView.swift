...
class TodayQuestionView: UIView {
  ...
  var question = ["극도로 심한 들뜸, 증상 조절 불가. 10", "극도로 들뜸, 환청이나 망상이 심함. 7", "일을 벌이고 기고만장하다. 4","오버한다, 나선다, 설친다. 3", "의욕, 자신감이 차있다. 2", "기분이 좋고 의욕적이다. 1", "기분이 보통이고 편안한 상태. 0", "의욕이 떨어지나, 할 일은 다 한다. -1", "좀 우울하고 기운이 떨어진다. -2", "꽤 우울하고 지친다. -3", "심하게 우울하고 처진다. -4", "극도로 우울하거나 처진다. -7", "극도로 심한 불안, 초조, 자살시도 -10"]
    ...
    private let thirdQuestionLabel = UILabel().then {
        $0.text = "3. 오늘을 떠올리고 아래 표에서 어울리는 나의 상태를 골라줘"
        $0.font = UIFont(name: "Avenir-Black", size: 13)
    }
